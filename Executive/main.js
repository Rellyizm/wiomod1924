...
client.on("messageCreate", async message => {
    ...
    //let embed = new Discord.MessageEmbed().setColor("#00ffdd").setAuthor({text: message.author.tag, url:  message.author.avatarURL({ dynamic: true, })}).setFooter({text: `Created by Relly.`}).setTimestamp();
...
const voiceUser = require("../Stat/src/schemas/voiceUser");
  client.on("interactionCreate", async (button) => {
    ...
    if (button.customId === "mesaj") {
      ...
      ${ayarlar.relly_chat} • __**Toplam Mesaj**__: ${messageData ? messageData.topStat : 0} Mesaj
      ${ayarlar.relly_chat} • __**Haftalık Mesaj**__: ${Number(messageWeekly).toLocaleString()} Mesaj 
      ${ayarlar.relly_chat} • __**Günlük Mesaj**__": ${Number(messageDaily).toLocaleString()} Mesaj

      ${ayarlar.relly_arama} **__Sohbet Ettiğin Text Kanalları__**: ${messageTop}
      `)], ephemeral: true });
    }
    ...
    !EglenceKategori.includes(x.id)
      ...
      ${ayarlar.relly_voice} • __**Toplam Ses**__: ${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika], s [saniye]")} Süre
      ${ayarlar.relly_voice} • __**Haftalık Ses**__: ${voiceWeekly} Süre
      ${ayarlar.relly_voice} • __**Günlük Ses**__: ${voiceDaily} Süre
