...
let embed = new MessageEmbed().setColor('RANDOM').setTimestamp().setFooter({text: 'Created by Relly.'})
...
const rellyData = await sunucuayar.findOne({ guildID: ayarlar.guildID })
await user.roles.add(rellyData.MAN).catch()
await user.roles.remove(rellyData.UNREGISTER).catch()
...
let modData = await modsData.findOne({guildID: channel.guild.id})
        ...
        await new registerData({ guildID: channel.guild.id, userID: user.id, Name: name, Sex: rellyData.MAN[0], Date: Date.now() }).save();
        ...
        if (modData && modData.nameMode === true) { channel.send({embeds: [embed.setDescription(`${user} üyesinin ismi başarıyla \`${name}\` olarak değiştirildi. Bu üye daha önce bu isimlerle kayıt olmuş. \n\n ${user.guild.emojis.cache.get(ayarlar.yes)} Kişinini toplamda **${x}** isim kayıtı bulundu. \n ${isimler} \n\n Kişinin önceki isimlerine \`.isimler @Relly/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)]}).sil(10); } else { channel.send({embeds: [embed.setDescription(`${user}, ${admin} tarafından başarıyla __**[ERKEK]**__ olarak kayıt edilmiştir.`)]}) }
        if (rellyData.TAG.some(s => user.user.username.includes(s) || user.user.discriminator.includes(s) || user.user.tag.includes(s))) await user.roles.add(rellyData.CREW).catch()

    }
    static async Woman(user, admin,name,channel) {
        const rellyData = await sunucuayar.findOne({ guildID: ayarlar.guildID })
        await user.roles.add(rellyData.WOMAN).catch()
        await user.roles.remove(rellyData.UNREGISTER).catch()
        await user.setNickname(name).catch()
        let data = await registerData.find({ guildID: channel.guild.id, userID: user.id })
        let x = data.length > 5 ? data.length - (data.length - 5): data.length
        let modData = await modsData.findOne({guildID: channel.guild.id})
                ...
                let adminData = await statData.findOne({ guildID: channel.guild.id, userID: admin.id })     
                let isimler = data.length > 0 ? data.map((value, i) => `\`${value.Name}\` (${user.guild.roles.cache.get(value.Sex)})`).slice(data.length > 5 ? data.length - 5 : 0, data.length).join('\n') : "Bu kullanıcı daha önce kayıt olmamış!"
                await new registerData({ guildID: channel.guild.id, userID: user.id, Name: name, Sex: rellyData.WOMAN[0], Date: Date.now() }).save();
                ...
                if (modData && modData.nameMode === true) { channel.send({embeds: [embed.setDescription(`${user} üyesinin ismi başarıyla \`${name}\` olarak değiştirildi. Bu üye daha önce bu isimlerle kayıt olmuş. \n\n ${user.guild.emojis.cache.get(ayarlar.yes)} Kişinini toplamda **${x}** isim kayıtı bulundu. \n ${isimler} \n\n Kişinin önceki isimlerine \`.isimler @Relly/ID\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.`)]}).sil(10); } else { channel.send({embeds: [embed.setDescription(`${user}, ${admin} tarafından başarıyla __**[KADIN]**__ olarak kayıt edilmiştir.`)]}) }
                if (rellyData.TAG.some(s => user.user.username.includes(s) || user.user.discriminator.includes(s) || user.user.tag.includes(s))) await user.roles.add(rellyData.CREW).catch()
   ...
   if(rellyData.MAN.some(s => member.roles.cache.has(s)) || rellyData.WOMAN.some(x => member.roles.cache.has(x))) return message.reply({embeds: [embed.setDescription(`${message.member}, Bu kullanıcı zaten kayıtlı.`)]}).sil(7);
   if(!rellyData.UNREGISTER.some(s => member.roles.cache.has(s))) return message.reply({embeds: [embed.setDescription(`${message.member}, Kullanıcıda kayıtsız rolü bulunmadığı için işlem gerçekleştirilemedi.`)]}).sil(7);
   ...
   if(data.tagMode == true) {
       if(rellyData.TAG.some(x => member.user.username.includes(x) || member.user.discriminator.includes(x)  || member.roles.cache.has(rellyData.VIP) || member.roles.cache.has(rellyData.BOOSTER))) {return false } else {
...
const rellyData = await sunucuayar.findOne({ guildID: ayarlar.guildID })           
if(!rellyData.REGISTERYetkiliRol.some(s => user.roles.cache.has(s)) && !user.permissions.has('ADMINISTRATOR') && !message.member.roles.cache.has(rellyData.BOTOWNER)) return true;
return false;
    }
