...
const sunucuayar = require("../Models/sunucuayar");
  ...
  let BotOwner = rellydata.BOTOWNER;
  let JailYetkili = rellydata.JailYetkiliRol;
  let Karantina = rellydata.CezalıRol;
  let JailLog = rellydata.JAILLOG;
  let Kayitsiz = rellydata.UNREGISTER;
...
Jaili Kaldırılma Tarihi: \`${moment(Date.now()).format("LLL")}\`
          `)
          message.guild.channels.cache.get(JailLog).wsend({ embeds: [log] }); //JAİLOG TANIMLA RELLY

};

exports.config = {
    name: "unjail",
    usage: `${ayarlar.BotPrefix}unjail (@Relly/ID)`,
    guildOnly: true,
    aliases: ["unjail"],
    cooldown: 3000
  }; 