...
if (!BanYetkili.some(x => message.member.roles.cache.has(x)) && !message.member.roles.cache.has(BotOwner)) return 
        ...
        let rellyseskontrol = message.guild.members.cache.filter(yetkili => !yetkili.user.bot && yetkili.roles.highest.position >= enAltYetkiliRolü.position).filter(yetkilises => !yetkilises.voice.channel && yetkilises.presence && yetkilises.presence.status != "offline")
        const yetkilisay = new Discord.MessageEmbed()
            ...
            .setFooter(`Created by Relly.`, message.guild.iconURL({ dynamic: true })).setTimestamp().setColor("RANDOM")
            .setDescription(`
...
`)
        ...
        message.channel.send(`${rellyseskontrol.map(yetkili => `${yetkili}`).join(', ')}`)
