...
module.exports = async (message) => {
	...
	const relly = await client.users.fetch("837686195877838889");
	const embed = new MessageEmbed()
		.setColor(message.member.displayHexColor)
		.setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, size: 2048 }))
		.setFooter("Created by Relly.", relly.avatarURL({ dynamic: true }))
		.setTimestamp();
	...
	if (!cmd || (cmd.conf.owner && !ayarlar.BotOwner.includes(message.author.id)) || !cmd.conf.enabled) return;
